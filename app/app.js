(function() {
        ...
        link: function(scope, element, attrs, controller) {
          ...
          leafletScope.$watch(
            ...
            function(newLegend) {
              if (isDefined(newLegend)) {
                legendClass = newLegend.legendClass
                  ? newLegend.legendClass
                  : "legend";
          ...
          var createLegend = function(map, legendData, newURL) {
            if (
              legendData &&
              legendData.layers &&
              legendData.layers.length > 0
            ) {
              if (isDefined(leafletLegend)) {
                leafletLegendHelpers.updateLegend(
                  leafletLegend.getContainer(),
                  legendData,
                  type,
                  newURL
                );
                ...
                leafletLegend.onAdd = leafletLegendHelpers.getOnAddLegend(
                  legendData,
                  legendClass,
                  type,
                  newURL
              ...
              if (
                isDefined(legend.loadedData) &&
                isFunction(legend.loadedData)
              ) {
                legend.loadedData();
              }
            }
          ...
          controller.getMap().then(function(map) {
            leafletScope.$watch("legend", function(newLegend) {
              ...
              if (
                !isDefined(newLegend.url) &&
                type === "arcgis" &&
                (!isArray(newLegend.colors) ||
                  !isArray(newLegend.labels) ||
                  newLegend.colors.length !== newLegend.labels.length)
              ) {
                $log.warn(
                  errorHeader + " legend.colors and legend.labels must be set."
                );
                return;
              }
              ...
              if (type === "arcgis") {
                leafletLegend.onAdd = leafletLegendHelpers.getOnAddArrayLegend(
                  newLegend,
                  legendClass
                );
              }
            ...
            leafletScope.$watch("legend.url", function(newURL) {
              ...
              if (!isArray(newURL) && !isString(newURL)) {
                $log.warn(
                  errorHeader + " legend.url must be an array or string."
                );
                return;
              }
              ...
              var onResult = function(idx, url) {
                return function(ld) {
                  if (isDefined(ld.data.error)) {
                    $log.warn(
                      errorHeader + "Error loadin legend from: " + url,
                      ld.data.error.message
                    );
                  } else {
                    if (
                      legendData &&
                      ...
                      legendData.layers = legendData.layers.concat(
                        ld.data.layers
                      );
                    } else {
                      legendData = ld.data;
            ...
            leafletScope.$watch(
              ...
              function(legendData) {
                ...
                if (
                  isDefined(leafletScope.legend.url) ||
                  !isDefined(legendData)
                ) {
                  return;
                }
  ...
  angular
    .module("ui-leaflet")
    .decorator("legendDirective", function legendDirectiveDecorator($delegate) {
      "ngInject";
      console.log(". . . . . . . . . . . . . .");
